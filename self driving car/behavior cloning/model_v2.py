...
from load_data_v2 import generate_data_batch, split_train_val
import sys
...
if __name__ == '__main__':
    ...
    train_data, val_data = split_train_val(csv_driving_data='data/driving_log_track2_and_track3.csv')
    ...
    nvidia_net.compile(optimizer='rmsprop', loss='mse')
    ...
    nvidia_net.fit_generator(
                         ...
                         samples_per_epoch=40*(len(train_data)/CONFIG['batchsize']+1),
                         nb_epoch=50,
                         validation_data=generate_data_batch(val_data, augment_data=False, bias=1.0),
                         nb_val_samples=5*(len(train_data)/CONFIG['batchsize']+1),
