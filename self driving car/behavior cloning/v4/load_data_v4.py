...
def load_data_batch(data, batchsize=CONFIG['batchsize'], data_dir='data', augment_data=True, bias=0.5):
    ...
    while loaded_elements < batchsize:
        ...
        ct_path, steer, throttle, brake, speed, time_stamp, lap, have_traffic_sign, sign_category  = shuffled_data.pop()

        # cast strings to float32
        steer = np.float32(steer)
        throttle = np.float32(throttle)
        have_traffic_sign = np.int(have_traffic_sign)
        ...
        if True:
            ...
            y_have_traffic_sign[loaded_elements] = have_traffic_sign
